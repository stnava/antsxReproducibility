...
excludeSigna = TRUE # eliminate the really bad scanner
...
mdl=(lm( outcome ~  age + Subject + Scanner + snr_total , data=dd ))
visreg::visreg( mdl, 'snr_total', gg=TRUE )
```
...
mdl=(lm( outcome ~  age + Subject + Scanner + snr_gm , data=dd ))
visreg::visreg( mdl, 'snr_gm', gg=TRUE )
```
...
library(lme4)
mdl0=(lm( outcome ~  age  +(Subject), data=dd ))
mycoffs0=coefficients(summary(mdl0))
mdl1=(lm( outcome ~  age  + Scanner + snr_total+ efc+(Subject), data=dd ))
summary(mdl1)
mycoffs1=coefficients(summary(mdl1))
# mdl=(lm( outcome ~  age   +(Subject), data=dd ))
ttl=paste0("best t=",
  format(mycoffs1["age",3],digits=3),
  " vs t=",format(mycoffs0["age",3],digits=3))
visreg::visreg( mdl1, 'age', gg=TRUE ) + ggtitle(ttl)
```
...
mdl=(lm( outcome ~  age + Subject + Scanner + efc , data=dd ))
grid.arrange( grobs = visreg::visreg( mdl, gg=TRUE ), ncol=1, main='reproducibility' )
```

## show volumes for one subject at different sites

```{r onesub3bars,fig.width=17,fig.height=6}
ggthemr("flat")
