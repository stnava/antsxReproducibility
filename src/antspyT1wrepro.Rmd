...
dfvols = dfvols[ !is.na(dfvols[,tarAnat]),]
dflong = pivot_longer( dfvols, tail(names(dfvols),1) )
...
varstostudy = varstostudy[ -grep("RandBasisProjPos",varstostudy)]
...
#        axis.text.x = element_text(angle=0, hjust=1))
library(plotly)
# fig <- plot_ly(relidf, type='bar', x = ~label, y = ~icc, text = ~label, name="",
#               hovertemplate = paste('%{x}', '<br>icc: %{text:.2s}<br>'),
#               texttemplate = '%{y:.2s}', textposition = 'outside')
# fig <- fig %>% layout(uniformtext=list(minsize=8, mode='hide'))
# fig

relidfvol = relidf[ grep("vol_",varstostudy),]
relidfarea = relidf[ grep("area_",varstostudy),]
relidfthk = relidf[ grep("thk_",varstostudy),]
...
volsel=relidfvol$mean>quantile(relidfvol$mean,0.5) &
 relidfvol$mean<quantile(relidfvol$mean,0.9)
